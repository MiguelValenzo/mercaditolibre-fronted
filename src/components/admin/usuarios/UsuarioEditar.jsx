...
const UsuarioEditar = ({ id, navegar }) => {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                const data = await apiService.getUsuario(id);
        ...
        if (id) {
            ...
            setError('No se proporcionó un ID de usuario válido para editar.');
            setCarga(false);
        }
    }, [id]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (cambiandoPassword && (!formData.password || formData.password.length < 6)) {
        ...
        try {
            ...
            if (cambiandoPassword && formData.password) {
            ...
            setError('Error al actualizar el usuario: ' + (err.message || 'Intente de nuevo.'));
        } finally {
            setGuardando(false);
        }
    };
            ...
            <div style={{ 
                ...
                {exito && (
                    <div style={{ 
                        ...
                        <span>¡Usuario actualizado exitosamente! Redirigiendo al panel...</span>
                    </div>
                )}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(420px, 1fr))', gap: '28px', marginBottom: '35px' }}>
                        ...
                        <div>
                            ...
                            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                                ...
                                >
                                    <option value="ROLE_CLIENTE" style={{ background: '#0f172a', color: '#f1f5f9' }}>Cliente</option>
                                    <option value="ROLE_ADMIN" style={{ background: '#0f172a', color: '#f1f5f9' }}>Administrador</option>
                                </select>
                            </div>
                            <div style={{ marginTop: '8px', fontSize: '12px', color: '#94a3b8', fontWeight: '500' }}>
                                {formData.rol === 'ROLE_ADMIN' 
                                    ...
                                    : '👤 El usuario podrá comprar productos y gestionar su perfil'}
                            </div>
                        </div>
                        ...
                        <div style={{ gridColumn: '1 / -1' }}>
                            <div style={{
                                ...
                                <Lock style={{ width: '18px', height: '18px', color: '#818cf8' }} />
                                <span style={{
                                    fontSize: '12px',
                                    fontWeight: '700',
                                    textTransform: 'uppercase',
